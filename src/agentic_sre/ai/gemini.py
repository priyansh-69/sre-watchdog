...
class GeminiProvider(BaseAIProvider):
    ...
    def __init__(
        self, api_key: Optional[str] = None, model_name: str = DEFAULT_GEMINI_MODEL
    ) -> None:
    ...
    def _get_client(self) -> Any:
        ...
        if self._client is None and self.api_key:
            try:
                ...
                logger.error(
                    f"[Agentic-SRE] Failed to initialize Google GenAI SDK client: {exc}"
                )
                return None
        return self._client

    def _get_fallback_response(
        self, crash_context: Dict[str, Any], reason: str
    ) -> Dict[str, Any]:
    ...
    async def analyze_error(self, crash_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.api_key:
            logger.warning(
                "[Agentic-SRE] GEMINI_API_KEY environment variable not set. Triggering fail-silent fallback."
            )
            return self._get_fallback_response(
                crash_context, "GEMINI_API_KEY not configured"
            )
        ...
        try:
            ...
            if client is None:
                return self._get_fallback_response(
                    crash_context, "Failed to initialize Gemini client"
                )
            ...
            if cleaned_text.startswith("```"):
                ...
                cleaned_text = re.sub(
                    r"\s*```$", "", cleaned_text, flags=re.IGNORECASE
                ).strip()
            ...
            if not isinstance(data, dict):
                logger.warning(
                    f"[Agentic-SRE] Gemini returned non-dictionary JSON: {type(data)}. Triggering fallback."
                )
                return self._get_fallback_response(
                    crash_context, "AI output format mismatch"
                )
            ...
            logger.error(
                f"[Agentic-SRE] Gemini Provider API exception: {exc}", exc_info=True
            )
            return self._get_fallback_response(
                crash_context, f"API error: {type(exc).__name__}"
            )
